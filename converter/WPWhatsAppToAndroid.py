...
def MoveDB():
    ...
    try:        
        with progressbar.ProgressBar(max_value=WPCallsCount) as bar:
            ...
            for row in calldb:
                ...
                # reg1 = (row[0], row[1], row[3], "call:" + row[2], 6, 0, None, row[4], None, None, 8, 0, None, None, 0, 0, 0.0, 0.0, None, None, row[5], -1, -1, -1, None, 0, None, None, None, None, None, 0, None, None, 0, None, None, 0, 0, None)
                # andb.execute("INSERT INTO messages VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", reg1)
                if row[5] is None:
                    ins = (row[0], LoopCount, row[4], video_call)
                    andb.execute("INSERT INTO missed_call_logs VALUES(?,?,?,?)", ins)
                else:
                    insert = (row[0], LoopCount, int(-1), row[4], video_call, duration, row[7], DataUsage)
    ...
    wpdb.execute('SELECT COUNT(MessageID) FROM Messages')
    WPMessageCount = wpdb.fetchone()[0]
    ...
    with progressbar.ProgressBar(max_value=WPMessageCount) as bar:
        BarCount5 = 0
        wpdb.execute("SELECT MessageID, KeyId FROM Messages")
        for rz in wpdb:
            key_ids_dict[rz[1]] = rz[0]
        ...
        for rc in wpdb:
            if rc[37] is not None:
                ...
                if quote_key:
                    ...
                    jid_key = None
                if jid_key is not None and (jid_key in key_ids_dict):
                    QuoteMsg.append(rc[0])
                    id_of = key_ids_dict[jid_key]
                    QuotedID.append(id_of)
                    ins7 = (rc[0], rc[3], jid_key, id_of)
        ...
        with progressbar.ProgressBar(max_value=WPMessageCount) as bar:
            ...
            for row in wpdb:
                if row[8] is not None:
                    if imghdr.what(None, row[8]) is not None:
                        thumb_image = row[8]
                    else:
                        thumb_image = None                
                        ...
                        if mentions is not None:
                            ...
                            for men in mentions:
                                ...
                                if LoopC == 0:
                                    ...
                                    mentioned_jids = (mentioned_jids + "," + men + "s.whatsapp.net")
                                LoopC = LoopC + 1
                # reg = (IDToCommit, row[1], row[2], row[3], row[4], 0, row[7], int(str(str(row[10]) + "000")), row[13], row[15], row[16], row[17], row[20], None, row[18], row[19], row[24], row[25], thumb_image, row[5], int(str(str(row[10] + 10) + "000")), -1, -1, -1, None, None, int(str(str(row[10] + 12) + "000")), None, row[22], row[34], row[35], 0, mentioned_jids, None, None, None, None, None, None, None)
                reg = (IDToCommit, row[1], row[2], row[3], row[4], 0, row[7], int(str(str(row[10]) + "000")), row[13], row[15], row[16], row[17], row[20], None, row[18], row[19], row[24], row[25], row[38], row[5], int(str(str(row[10] + 10) + "000")), -1, -1, -1, None, None, int(str(str(row[10] + 12) + "000")), None, row[22], row[34], row[35], 0, mentioned_jids, None, None, None, None, None, None, None)
                if row[0] in QuotedID:
                    andb.execute("INSERT INTO messages VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", reg)
                    # rego = (None, row[1], row[2], row[3], row[4], 0, row[7], int(str(str(row[10]) + "000")), row[13], row[15], row[16], row[17], None, row[22], None, row[18], row[19], row[24], row[25], thumb_image, row[5], int(str(str(row[10] + 10) + "000")), -1, -1, -1, None, None, None, 0, row[34], row[35], 0, mentioned_jids, None, 0, None, None, None, None, None)
                    rego = (None, row[1], row[2], row[3], row[4], 0, row[7], int(str(str(row[10]) + "000")), row[13], row[15], row[16], row[17], None, row[22], None, row[18], row[19], row[24], row[25], row[38], row[5], int(str(str(row[10] + 10) + "000")), -1, -1, -1, None, None, None, 0, row[34], row[35], 0, mentioned_jids, None, 0, None, None, None, None, None)
